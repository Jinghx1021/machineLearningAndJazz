function create_prediction_song3(batch)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~(strcmp(files(i,1).name,'.'))&& ~(strcmp(files(i,1).name,'..'))&& ~(strcmp(files(i,1).name,'.DS_Store'))  %if to by pass . and .. DOS comands listed by dir as files
       ...
       %???         emb_song_left=remove_instances(emb_all,song_out_emb_idx);
                ...
                remove_idx = [...
                    ...
                    49,...energyDevNom
                    ...back to PAS
                    50,...energyDev (this should be calculated before in PAS section
                    ...FileName
                    51,...FileName(for information)
...
%                    note_v=test_m(note_id,:);%get note descriptors
                    ...
                    if strcmp(emb_pred(note_i),'y')%note emb pred = y
                         ...
                         closest_emb=find(ismember(idxp,train_emb_n_idx));%find indexes of the closest notes which are not ornamented
                         idx=idxp(closest_emb(1));%get the closest not embellished
                    end
