function create_prediction_song_app_1_seq_mod2(batch)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
classifiers={'ANN','KNN','SVM','Trees'};

for i=1:length(classifiers);
    predict_song(j,ia1,transformation_all,score_all_pa,plot_op, files,numberOfFiles, classifiers{i});
end
end

function predict_song(j,ia1,transformation_all,score_all_pa,plot_op, files, numberOfFiles, classifier)
...
for file_i=1:numberOfFiles, %for each file (do not count . and ..
    if ~(strcmp(files(file_i,1).name,'.'))&& ~(strcmp(files(file_i,1).name,'..'))&& ~(strcmp(files(file_i,1).name,'.DS_Store'))  %if to by pass . and .. DOS comands listed by dir as files
        ...
        if strcmp(files(file_i,1).name(end-2:end),'xml') %filter xml files only
            ...
            fprintf('Performing transformation based on %s prediction for %s\n',classifier,files(file_i,1).name);            
...
%            end
            ...
            %% Load Predictions
            emb_pred = textread([pwd,'/dataOut/arffs/leaveOneOut/embellishment/predictions/',files(file_i,1).name(1:end-4),'_',classifier,'.txt'],'%s');
            durRat_pred = textread([pwd,'/dataOut/arffs/leaveOneOut/durRat/predictions/',files(file_i,1).name(1:end-4),'_',classifier,'.txt'],'%f');
            energyRat_pred = textread([pwd,'/dataOut/arffs/leaveOneOut/energyRat/predictions/',files(file_i,1).name(1:end-4),'_',classifier,'.txt'],'%f');
            onsetDev_pred = textread([pwd,'/dataOut/arffs/leaveOneOut/onsetDev/predictions/',files(file_i,1).name(1:end-4),'_',classifier,'.txt'],'%f');
%            pitchDev_pred = textread([pwd,'/dataOut/arffs/leaveOneOut/pitchDev/predictions/',files(file_i,1).name(1:end-4),'_',classifier,'.txt'],'%f');
...
%                 end
                if ~isempty(child_nmat_performed_idx) %if ornament is not "omitt note"
                    ...
                    %do concatenation = false....
                    %%out_wav = concatenate_audio(idx,ins_idx,score_songs_left, nmat_performed,child_nmat_performed_idx,emb_pred,nmat_score, score_song_out,nmat_learned,note_i,plot_op);
                    fprintf('done!\r')
                else
                fprintf('note ommited... done!\r');    
                end
            end
            ...
            writemidi_java(nmat_learned,[pwd,'/dataOut/learntSongs/',classifier,'/',files(file_i,1).name(1:end-4),'.mid'],120,score_song_out.tempo(1),tsig1,tsig2);
            %writemidi(nmat_learned,[pwd,'/dataOut/learntSongs/approach_1/',files(j,1).name(1:end-4),'.mid'],120,score_song_out.tempo(1));
            save ([pwd,'/dataOut/learntSongs/',classifier,'/',files(file_i,1).name(1:end-4),'.mat'],'nmat_learned');
%            audiowrite([pwd,'/dataOut/learntSongs/',classifier,'/',files(file_i,1).name(1:end-4),'.wav'],out_wav,fs);
            save ([pwd,'/dataOut/learntSongs/',classifier,'/',files(file_i,1).name(1:end-4),'_p2s.mat'],'p2s');            
...
function out_wav = concatenate_audio(idx,ins_idx,score_songs_left, nmat_performed,child_nmat_performed_idx,emb_pred,nmat_score, score_song_out,nmat_learned,note_i,plot_op)
